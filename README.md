...
#This example contemplates only 1 secuence
...
leng_sorted = ds.filt(data=df, leng=10, id="gene", column="secuence")
...
genome = ds.open_file("genome.tsv") #with ID : secuences
...
- Genome.RNA(complement="", reverse="")
    Converts DNA secuence to RNA if not argument given.
    If complement=True: returns the RNA complementary.
    If reverse=True: returns the secuence reversed.

- Genome.GC_content(an="", rd="")
    Returns the GC content as list if no arguments given:
        {"ID": [#G, #C]}
    If an="%": returns the percentage of GC in secuence.
